...
async def _index_repo(
    ...
    await registry.update(repository_id, status=IndexStatus.INDEXING.value)
    try:
        ...
        await registry.update(
            repository_id, status=IndexStatus.FAILED.value, error=str(exc)
        )
...
async def create_repository(
    ...
    record = await registry.create(req.url, req.branch)
    service = RepositoryService(rag, llm)
    background.add_task(_index_repo, record.id, req.url, req.branch, service)
...
async def get_repository(repository_id: str) -> RepositoryResponse:
    ...
    record = await registry.get(repository_id)
    if record is None:
...
async def delete_repository(repository_id: str) -> dict[str, str]:
    """Remove a repository from the registry."""
    if not await registry.delete(repository_id):
        raise NotFoundError(f"Repository {repository_id} not found")
...
async def list_repositories() -> list[RepositoryResponse]:
    ...
    return [_to_response(record) for record in await registry.list()]


@router.post("/{repository_id}/chat", response_model=Answer)
async def repository_chat(
    repository_id: str, req: RepoChatRequest, rag: RagDep, llm: LLMDep
) -> Answer:
